...
describe('/test/check.access.test.js', () => {
  ...
  describe('check access', () => {
    it('access denied brands', async () => {
      const response = await fetch(`http://localhost:${config.server.port}/api/bridge/brands`)
        .then(_getData);
      expect(response.status, 'сервер возвращает статус 401').to.be.equal(401);
    });
    it('access denied providers', async () => {
      const response = await fetch(`http://localhost:${config.server.port}/api/bridge/providers`)
        .then(_getData);
      expect(response.status, 'сервер возвращает статус 401').to.be.equal(401);
    });
    it('access denied upload file', async () => {
      const response = await fetch(`http://localhost:${config.server.port}/api/bridge/file/upload`, { method: 'POST' })
        .then(_getData);
      expect(response.status, 'сервер возвращает статус 401').to.be.equal(401);
    });
